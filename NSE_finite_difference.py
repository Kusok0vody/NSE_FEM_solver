    ...
    for k in range(0, frames_num-1):
        ...
        for i in range(0, size_x-1):
            for j in range(0, size_y-1):
                ...
                tilda_v[i][j] = v[k][i][j] + dt * (Force_y[k][i][j] + nu*laplace_v - 0*nabla_v)
        ...
        for i in range(0, size_x-1):
            for j in range(0, size_y-1):
                ...
                divergence = density * (dx * dy**2 * (tilda_u[i+1][j] - tilda_u[i][j]) + dx**2 * dy * (tilda_v[i][j+1] - tilda_v[i][j])) / dt    
                p[k][i][j] = laplace_p - divergence + p[k][i][j]

        # third step: calculating true velocities with right pressure
        for i in range(0, size_x-1):
            for j in range(0, size_y-1):
                u[k+1][i][j] = tilda_u[i][j] - dt * ((p[k][i][j] - p[k][i-1][j]) / dx) / density
                v[k+1][i][j] = tilda_v[i][j] - dt * ((p[k][i][j] - p[k][i-1][j]) / dy) / density
